...
library(leaps)
#full model
full_mod <- lm(price ~ mileage + engine_power + age + is_new_energy + is_common_color, data = bmw_clean)
...
models <- list(
  "Five Predictors" = lm(price ~ mileage + engine_power + age + is_new_energy + is_common_color, data = bmw_clean),
  "Four Predictors" = lm(price ~ mileage + engine_power + age + is_new_energy, data = bmw_clean),
  "Three Predictors" = lm(price ~ mileage + engine_power + age, data = bmw_clean),
  "Two Predictors" = lm(price ~ mileage + engine_power, data = bmw_clean),
  "One Predictors" = lm(price ~ mileage, data = bmw_clean)
)
...
```{r stepwise_Mallows}
fwd <- leaps::regsubsets(price ~ mileage + engine_power + age + is_new_energy + is_common_color, 
                         data = bmw_clean, 
                         method = "forward",
                         nvmax = 5)
summary(fwd)$which
...
#bar graph
cp %>% ggplot(aes(x = predictors, y = summary(fwd)$cp)) + 
