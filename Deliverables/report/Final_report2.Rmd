...
```{r}
new_mod <- lm(log(price) ~ I(mileage^(0.5)) + engine_power + I(age^(0.5)) + is_new_energy + numSeats + feature_1 + feature_2 + feature_3 +  feature_4 + feature_5 + feature_6 + feature_8, data = training)
...
#plot of y vs fitted values
par(mfrow=c(1,1))
plot(new_mod$fitted.values,log(price),xlab="Fitted Values")
...
test = data.frame(log(validation$price),output$fit, 1:length(output$fit));
