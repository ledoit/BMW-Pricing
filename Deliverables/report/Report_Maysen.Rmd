...
invResPlot(best_mod)
#the power curve with a lambda of 0.69 provides closest fit to data, we will round to 0.7
```


```{r}
#new mod
m2 <- lm(log(price)~log(mileage)+engine_power+log(age)+I(age^2)+is_new_energy+numSeats+feature_1+feature_2+feature_3+feature_4+feature_5+feature_6+feature_8, data = training)

#plot of y vs fitted values
par(mfrow=c(1,1))
plot(m2$fitted.values,log(training$price),xlab="Fitted Values")
...
pairs(I(price^(0.7))~mileage+engine_power+age, data = training)
...
#MSE for validation is very close to MSE of training data
```
