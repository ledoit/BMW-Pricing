...
```
```{r}
hats <- as_tibble(hatvalues(best_mod))
cutoff <- 4/2419
hats %>% filter(value>cutoff)
...
```{r}
#new mod
m2 <- lm(I(price^(0.7))~mileage+I(mileage^(2))+engine_power+age+I(age^(2))+is_new_energy+numSeats+feature_1+feature_2+feature_3+feature_4+feature_5+feature_6+feature_8, data = training)

#plot of y vs fitted values
par(mfrow=c(1,1))
plot(m2$fitted.values,training$price^(0.7),xlab="Fitted Values")
