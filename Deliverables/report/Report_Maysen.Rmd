...
df <- data.frame(Predictor = c("is_new_energy", "is_common_color", "feature_1", "feature_2", "feature_3", "feature_4", "feature_5", "feature_6", "feature_7", "feature_8"),
                 Difference_in_Means = c(en_diff, col_diff, f1_diff, f2_diff, f3_diff, f4_diff, f5_diff, f6_diff, f7_diff, f8_diff))
...
mod <- lm(price~mileage+engine_power+age, data = training)
avPlots(mod, col = "blue", col.lines = "black", lwd = 2)
```

```{r}
#diagnostic plots
m1 <- lm(price~mileage+engine_power+age+is_new_energy+feature_1+feature_2+feature_3+feature_4+feature_5+feature_6+feature_8, data = training)
par(mfrow=c(2,2))
plot(m1)
...
par(mfrow=c(1,1))
plot(m1$fitted.values,training$price,xlab="Fitted Values")
#produces quadratic rather than linear trend suggesting we consider a transformation on Y

#scatter plot matrix of data

pairs(price~mileage+engine_power+age,data = bmw_clean)

#pairs(price~mileage+engine_power+age)
...
invResPlot(m1)
#the power curve with a lambda of 0.66 provides closest fit to data, we will round to 0.7
```


```{r}
#new mod
m2 <- lm(I(price^(0.7))~mileage+engine_power+age+is_new_energy+feature_1+feature_2+feature_3+feature_4+feature_5+feature_6+feature_8, data = training)

#plot of y vs fitted values
par(mfrow=c(1,1))
plot(m2$fitted.values,training$price^(0.6),xlab="Fitted Values")
...
plot(training$mileage,StanRes2,ylab="Standardized Residuals")
plot(training$engine_power,StanRes2,ylab="Standardized Residuals")
plot(training$age,StanRes2,ylab="Standardized Residuals")
plot(m2$fitted.values,StanRes2,ylab="Standardized Residuals",xlab="Fitted Values")

#scatter plot matrix of data

pairs(I(price^(0.6))~mileage+engine_power+age, data = bmw_clean)

#pairs(I(price^(0.6))~mileage+engine_power+age)
```
